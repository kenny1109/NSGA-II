...
class NSGA2Utils:
    ...
    def fast_nondominated_sort(self, population):
        ...
        for individual in population:
            individual.domination_count = 0  # np=0
            individual.dominated_solutions = []  # sp = ∅
            for other_individual in population:
                if individual.dominates(other_individual):
                    individual.dominated_solutions.append(other_individual)
                elif other_individual.dominates(individual):
                    individual.domination_count += 1
            if individual.domination_count == 0:  # 找出当前最优PF的解
                individual.rank = 0
        ...
        while len(population.fronts[i]) > 0:
            ...
            for individual in population.fronts[i]:
                for other_individual in individual.dominated_solutions:
                    ...
                    if other_individual.domination_count == 0:  # 对个体根据占优情况进行perato front分层
    ...
    def calculate_crowding_distance(self, front):
        if len(front) > 0:
            ...
            for individual in front:
                individual.crowding_distance = 0  # 距离初始化为0

            for m in range(len(front[0].objectives)): # 对每一个目标函数进行排序
                # lambda作为一个表达式 individual为变量 individual.objectives[m]为函数
    ...
    def __tournament(self, population):
        participants = random.sample(population.population, self.num_of_tour_particips)  # 从指定序列中随机获取指定长度的片断
        best = None
